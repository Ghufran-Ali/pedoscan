...
def plot_heatmap(frame_idx=-1, data=None, save=True):
    ...
    fig.suptitle(
        f"BILATERAL FOOT PRESSURE ANALYSIS  ·  Frame {frame_idx + 1}/{len(data)}",
    ...
    cbar_ax_left = fig.add_axes([0.17, 0.05, 0.01, 0.3])
    ...
    cbar_ax_right = fig.add_axes([0.24, 0.05, 0.01, 0.3])
    ...
    ax_donut_r = fig.add_subplot(gs[0, 2])
    ...
    ax_donut_l = fig.add_subplot(gs[0, 3])
    ...
    ax_arch = fig.add_subplot(gs[1, 2:4])
    ...
    ax_diab = fig.add_subplot(gs[2, 2:4])
...
if __name__ == "__main__":
    ...
    print(f"\n{'='*60}")
    print(f"  BILATERAL LARGE VELOSTAT FOOT PRESSURE SYSTEM")
    print(f"{'='*60}")
    print(f"  Total sensors   : 32 (16 per foot)")
    print(f"    • Right Foot   : 16 sensors")
    print(f"    • Left Foot    : 16 sensors (mirrored)")
    print(f"  Sensor sizes    : ~20-25mm diameter each (LARGE Velostat)")
    ...
    print("\nSESSION SUMMARY")
    print(f"  Avg arch pressure (R/L)        : {np.mean([r['zone_avg']['Midfoot'] for r in all_right]):.1f} / {np.mean([l['zone_avg']['Midfoot'] for l in all_left]):.1f}")
    print(f"  Frames with flat foot (mod+) R : {sum(1 for r in all_right if r['flat_foot_risk'] >= 2)}/{len(data)}")
    print(f"  Frames with flat foot (mod+) L : {sum(1 for l in all_left if l['flat_foot_risk'] >= 2)}/{len(data)}")
    print(f"  Frames with High Diabetic Risk : {sum(1 for r in all_right if r['diabetic_risk'] == 'High Risk')}/{len(data)} (R)  {sum(1 for l in all_left if l['diabetic_risk'] == 'High Risk')}/{len(data)} (L)")